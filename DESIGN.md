...
- **The subtitle was sized like a UI caption, not like a subtitle.** 21px on a 798px composed frame is **2.6%** of picture height. Broadcast and streaming captions sit near 4% — YouTube's default is ~4.2%, Netflix's 1080p spec ~3.9% — and ~2% is roughly where type stops surviving a downscale. Gambit's whole output is a recording, so the cue is read at delivery size, not authoring size, and a 42-character line was using only 58% of the board's width. It is now 29px (3.6%), which is where two constraints meet: the track's content box is 46px, so 32px (a true 4%) leaves 2.5px of slack and looks wedged into its own strip, while the hard ceiling is 33.8px — past that a *one-line* cue grows the 64px track that present mode's floating-bar floor is derived from.

  Two couplings surfaced only by measuring, not by reasoning. On the 560px short-desktop frame the same cue wraps to two lines at 29px, doubling the track to 96px and dropping the floating bar onto the subtitles (−36px); the cue is part of the artifact, so it scales with it (22px there). And the present floor itself was measured against a *one-line* track: a two-line cue at 1440×920 already had exactly **0px** of clearance before any of this. Two lines is the cap in every caption standard, so that is what the floor is measured against now — and measured on the page rather than derived, because the app's top padding shifts across height tiers and the arithmetic predicts 958 where the browser says 970. That recovers common laptop viewports (1470×956) that were silently clipping.
...
**The current row drops the color.** That pill is studio-steel-blue, where all four marks measure 1.20–1.72:1 and no lightening rescues them, so the mark inherits the row's chalk instead — the same move `.pgn-varnum` already makes. Nothing is lost: the `!!` / `??` still reads, and the board badge is showing the color full size at that exact moment. The four hexes live only in `annotationMarkColors`; the mark takes its color inline the way a seek dot takes `markerColors[kind]`, so there is no CSS custom property and no per-kind class for it to disagree with.
